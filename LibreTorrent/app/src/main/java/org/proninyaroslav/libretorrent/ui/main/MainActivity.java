...
public class MainActivity extends AppCompatActivity implements FragmentCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /*        ADD on            */
        // Retrieve the last usage time from SharedPreferences
        SharedPreferences prefs = getSharedPreferences("my_prefs", MODE_PRIVATE);
        long lastUsageTime = prefs.getLong("last_usage_time", 0);

        // Display the last usage time as a toast message
        if (lastUsageTime != 0) {
            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault());
            String lastUsageTimeString = dateFormat.format(lastUsageTime);
            Toast.makeText(this, "Last usage: " + lastUsageTimeString, Toast.LENGTH_SHORT).show();
        }
        /*        ADD on            */
...
//    added
    ...
    protected void onPause() {
        ...
        // Save the current time as the last usage time in SharedPreferences
        SharedPreferences prefs = getSharedPreferences("my_prefs", MODE_PRIVATE);
        SharedPreferences.Editor editor = prefs.edit();
        editor.putLong("last_usage_time", System.currentTimeMillis());
        editor.apply();
    }
    //added
    @Override
